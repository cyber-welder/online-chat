...
class ServerStorage:
    ...
    * дата последнего входа (last login_time).
    ...
    class UserMixin:
        """
        Создал миксин для использования в таблицах с историей и активных пользователях,
        так как там используются одинаковые поля, DRY
        """
    ...
    * port
    """
    class UserHistory(UserMixin, Base):
    ...
    * login_time.
    ...
    class UserActive(UserMixin, Base):
...
if __name__ == "__main__":
    db = ServerStorage()
    db.init_user("client", "127.0.0.1", 7777)
    db.init_user("client1", "127.0.0.1", 7777)
    db.destroy_user_session("client")
